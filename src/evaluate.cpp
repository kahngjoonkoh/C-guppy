...
int Position::pesto_eval() noexcept {
    int mg[2];
    int eg[2];
    int gamePhase = 0;
    ...
    for (int y = 7; y>=0; y--){
        for (int x = 0; x < 8; ++x) {
            ...
            const auto side = static_cast<bool>(occupancy(Side::Black) & bb);
            const auto pc = piece * 2 + static_cast<int>(side);

            if (piece != Piece::None) {
                mg[side] += mg_table[pc][i];
                eg[side] += eg_table[pc][i];
    ...
    int egScore = eg[turn2play] - eg[!turn2play];
    int mgPhase = gamePhase;
    if (mgPhase > 24) mgPhase = 24; /* in case of early promotion */
    int egPhase = 24 - mgPhase;

//    std::cout<<(mgScore * mgPhase + egScore * egPhase) / 24<<std::endl;
